...
interface SystemPositionRepository : JpaRepository<SystemPosition, Short> {
    ...
    @Query("select p.* from system_position p where p.status=true and not exists(select 1 from system_dept_position x where p.id=x.position_id and x.dept_id=:deptId)", nativeQuery = true)
    fun findAvailablePositions(deptId: Short): List<SystemPosition>

    @Query("select p.* from system_position p where p.status=true and exists(select 1 from system_dept_position x where p.id=x.position_id and x.dept_id=:deptId)", nativeQuery = true)
